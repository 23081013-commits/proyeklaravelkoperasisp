    ...
    <div class="card-body">
        <form action="{{ route('angsuran.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="jumlah_bayar" id="jumlah_bayar" class="form-control @error('jumlah_bayar') is-invalid @enderror" placeholder="Contoh: 250000" value="{{ old('jumlah_bayar') }}">
                @error('jumlah_bayar')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
